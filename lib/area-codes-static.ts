// Static state → area-code mapping derived from area-codes.txt
...
const RAW: Record<string, string[]> = {
  ...
  "Florida":              ["239","305","321","352","386","407","561","727","754","772","786","813","850","863","904","954"],
  "Georgia":              ["229","404","470","478","668","706","762","770","912"],
  "Hawaii":               ["808"],
  "Idaho":                ["208"],
  ...
  "North Carolina":       ["336","704","743","828","910","919","980","984"],
  "North Dakota":         ["701"],
...
export const AREA_CODES_BY_STATE: Record<string, string[]> = RAW

// code → state name
const CODE_TO_STATE_MAP: Record<string, string> = {}
for (const [state, codes] of Object.entries(RAW)) {
  for (const code of codes) CODE_TO_STATE_MAP[code] = state
}
export const CODE_TO_STATE: Record<string, string> = CODE_TO_STATE_MAP

// slug → state name
const SLUG_TO_STATE_MAP: Record<string, string> = {}
for (const state of Object.keys(RAW)) SLUG_TO_STATE_MAP[stateToSlug(state)] = state

export function slugToState(slug: string): string | undefined {
  return SLUG_TO_STATE_MAP[slug]
}

export function getAllStateSlugs(): string[] {
  return Object.keys(RAW).map(stateToSlug)
}

export function getCodesByStateSlug(stateSlug: string): string[] {
  const state = slugToState(stateSlug)
  if (!state) return []
  return AREA_CODES_BY_STATE[state] ?? []
}
