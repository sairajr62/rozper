...
// The mapping is generated by scripts/upload-blog-to-blob.mjs.

let _blobUrls: Record<string, string> | null = null

function getBlobUrls(): Record<string, string> {
  if (_blobUrls) return _blobUrls
  const jsonPath = path.join(process.cwd(), "lib", "blog-blob-urls.json")
  try {
    _blobUrls = JSON.parse(fs.readFileSync(jsonPath, "utf8"))
  } catch {
    _blobUrls = {}
  }
  return _blobUrls!
...
function resolveBlobUrl(localPath: string): string {
  ...
  // Strip "-cover" suffix and retry: e.g. "312-area-code-cover.webp" → "312-area-code.webp"
  const withoutCover = filename.replace(/-cover(\.[^.]+)$/, "$1")
  if (withoutCover !== filename && blobUrls[withoutCover]) return blobUrls[withoutCover]!

  // No blob URL found — return original path (will fall back to local file or CoverArt)
  return localPath
}
