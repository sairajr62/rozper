...
import {
  Shield, Lock, Globe, Users, Settings, BarChart3,
  ArrowRight, ChevronRight, Building2, CheckCircle,
} from 'lucide-react'

const features = [
  { icon: Lock, title: 'SSO & SCIM', desc: 'SAML 2.0 SSO and SCIM provisioning — connect your IdP, automate user lifecycle.' },
  { icon: Globe, title: 'Multi-Site Routing', desc: 'Route across global contact center sites with priority and overflow rules.' },
  { icon: Shield, title: 'Custom SLAs', desc: 'Negotiate SLA tiers per team, queue, or customer segment — backed by our team.' },
  { icon: Users, title: 'Dedicated Success Team', desc: 'Named CSM and 24/7 priority support with 1-hour response SLA.' },
  { icon: Settings, title: 'Custom Integrations', desc: 'Custom API integrations, webhook pipelines, and enterprise data export.' },
  { icon: BarChart3, title: 'Advanced Compliance', desc: 'SOC 2, HIPAA, GDPR, PCI DSS — with audit logs and data residency options.' },
]

const faqs = [
  { q: 'What does the enterprise onboarding look like?', a: 'Dedicated implementation manager, custom training, SSO setup, and a 30-day hypercare period with white-glove support.' },
  { q: 'Can we bring our own PSTN carrier?', a: 'Yes. Bring Your Own Carrier (BYOC) lets you connect existing SIP trunks or carrier contracts to Rozper.' },
  { q: 'What compliance certifications are included?', a: 'SOC 2 Type II, HIPAA BAA, GDPR DPA, PCI DSS SAQ-D. Audit logs and data residency available on request.' },
]
  ...
  return (
    <div className="rounded-3xl bg-[#111B2D] border border-white/10 overflow-hidden">
      <div className="px-5 py-3 bg-[#0B1220] border-b border-white/10 flex items-center gap-2">
        ...
        <span className="text-xs font-mono text-white/60">Enterprise Plan · Acme Corp</span>
        <div className="ml-auto px-2 py-0.5 rounded bg-emerald-400/10 border border-emerald-400/20 text-[9px] font-mono text-emerald-400">Active</div>
      </div>

      {/* SSO config */}
      <div className="p-5 border-b border-white/5">
        <div className="text-[10px] font-mono text-white/30 uppercase tracking-widest mb-3">Identity Provider</div>
        <div className="flex items-center gap-3 p-3 rounded-xl bg-[#0B1220] border border-[#046BD2]/20">
          ...
          <div>
            <div className="text-xs font-medium text-white">Okta SSO — Connected</div>
            <div className="text-[10px] font-mono text-[#0086F9]/60">SAML 2.0 · 847 users synced</div>
          </div>
      ...
      {/* Multi-site */}
      <div className="p-5 border-b border-white/5">
        <div className="text-[10px] font-mono text-white/30 uppercase tracking-widest mb-3">Active Sites</div>
        <div className="space-y-2">
          {[
            ...
            <motion.div key={site.name} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex-1">
                ...
                <div className="text-[10px] font-mono text-white/30">{site.agents} agents</div>
              </div>
              <span className={`text-[9px] font-mono px-2 py-0.5 rounded border ${site.status === 'Primary' ? 'text-[#0086F9] border-[#046BD2]/25 bg-[#046BD2]/10' : 'text-white/30 border-white/10 bg-white/5'}`}>
                {site.status}
              </span>
            </motion.div>
          ))}
      ...
      {/* Checklist */}
      <div className="p-5">
        <div className="text-[10px] font-mono text-white/30 uppercase tracking-widest mb-3">Enterprise Features</div>
        <div className="grid grid-cols-2 gap-2">
          {enterpriseFeatures.map((f, i) => (
            <motion.div key={f.label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 + 0.3 }}
              className="flex items-center gap-2">
              <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
              <span className="text-[10px] font-mono text-white/55">{f.label}</span>
            </motion.div>
          ))}
        </div>
      </div>
  ...
  return (
    <main className="min-h-screen bg-[#0B1220] overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-[0.015]" style={{
        backgroundImage: 'linear-gradient(rgba(0,134,249,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(0,134,249,0.4) 1px, transparent 1px)',
      ...
      <div className="relative z-10">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 pb-16 grid lg:grid-cols-[1.1fr_1fr] gap-12 items-start">
          <div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#046BD2]/10 border border-[#046BD2]/30 mb-8">
              ...
              <span className="text-xs font-mono uppercase tracking-widest text-[#2D98F1]">Contact Center · Enterprise</span>
            </motion.div>
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.1 } }} className="font-display text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.05] tracking-tight">
              Enterprise rules.<br />
              <span className="bg-gradient-to-r from-[#046BD2] via-[#0086F9] to-[#2D98F1] bg-clip-text text-transparent">Your terms</span>.
            </motion.h1>
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.2 } }} className="mt-6 text-lg text-white/60 max-w-xl leading-relaxed">
              SSO, SCIM, multi-site routing, custom SLAs, and a dedicated success team. Contact center built for enterprises that need control.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.3 } }} className="mt-10 flex flex-wrap gap-3">
              <Link href="/contact" className="group inline-flex items-center gap-2 px-7 py-4 rounded-full bg-[#046BD2] hover:bg-[#0078E0] text-white font-semibold transition">
                Talk to enterprise sales <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-24">
          <div className="text-center mb-16">
            <h2 className="font-display text-4xl md:text-5xl font-bold">Enterprise-grade. Without the enterprise pain.</h2>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-24">
          <div className="relative rounded-[2.5rem] overflow-hidden p-12 md:p-20 text-center bg-gradient-to-br from-[#046BD2] via-[#0078E0] to-[#0086F9]">
            ...
            <h2 className="font-display text-4xl md:text-6xl font-bold text-white">Built for your scale. Ready on day one.</h2>
