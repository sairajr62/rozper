...
import {
  Eye, Mic, Radio, AlertTriangle, BarChart3, Shield,
  ArrowRight, ChevronRight, Users,
} from 'lucide-react'

const features = [
  { icon: Eye, title: 'Silent Monitor', desc: 'Listen to any active call without the agent or customer knowing.' },
  { icon: Mic, title: 'Whisper Coaching', desc: 'Coach agents mid-call — they hear you, the customer doesn\'t.' },
  { icon: Radio, title: 'Barge-In', desc: 'Join any call as a participant the moment a situation escalates.' },
  { icon: AlertTriangle, title: 'AI Escalation Alerts', desc: 'Instant alert when sentiment, keywords, or duration cross your threshold.' },
  { icon: BarChart3, title: 'Real-Time Dashboards', desc: 'Live queue metrics, agent status, and CSAT predictions on one screen.' },
  { icon: Shield, title: 'Compliance Flags', desc: 'Automatic alerts for prohibited language, missing disclosures, or policy violations.' },
]

const faqs = [
  { q: 'Do agents know when they\'re being monitored?', a: 'In silent mode, no. In whisper mode, the agent hears you but the customer doesn\'t. Barge-in is fully audible.' },
  { q: 'Can I set team-specific alert thresholds?', a: 'Yes. Configure sentiment, keyword, duration, and hold time alerts per queue or individual agent.' },
  { q: 'How many agents can a supervisor monitor?', a: 'All active agents are visible on the live dashboard simultaneously. Listen to one call at a time.' },
]

const agents = [
  { name: 'Priya N.', queue: 'Sales', status: 'On Call', duration: '5:14', sentiment: 91 },
  { name: 'Marcus L.', queue: 'Support', status: 'On Call', duration: '14:22', sentiment: 38, alert: true },
  { name: 'Layla H.', queue: 'Sales', status: 'Available', duration: '—', sentiment: null },
  { name: 'Jordan K.', queue: 'Billing', status: 'On Call', duration: '3:07', sentiment: 76 },
  { name: 'Sam T.', queue: 'Support', status: 'Wrap-Up', duration: '1:02', sentiment: null },
  { name: 'Diego F.', queue: 'Enterprise', status: 'On Call', duration: '6:48', sentiment: 88 },
]
  ...
  return (
    <div className="rounded-3xl bg-[#111B2D] border border-white/10 overflow-hidden">
      ...
      <div className="px-5 py-3 bg-red-400/5 border-t border-red-400/10 flex items-center gap-2">
        ...
        <span className="text-[10px] font-mono text-red-400">Marcus L. — sentiment dropping · 14min call · click to listen</span>
      </div>
    </div>
  )
}
      ...
      <div className="relative z-10">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 pb-16 grid lg:grid-cols-[1fr_1.1fr] gap-12 items-start">
          <div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#046BD2]/10 border border-[#046BD2]/30 mb-8">
              ...
              <span className="text-xs font-mono uppercase tracking-widest text-[#2D98F1]">Contact Center · Supervisor Tools</span>
            </motion.div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-24">
          <div className="text-center mb-16">
            <h2 className="font-display text-4xl md:text-5xl font-bold">Total visibility. Real-time control.</h2>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-24">
          <div className="relative rounded-[2.5rem] overflow-hidden p-12 md:p-20 text-center bg-gradient-to-br from-[#046BD2] via-[#0078E0] to-[#0086F9]">
            ...
            <h2 className="font-display text-4xl md:text-6xl font-bold text-white">Monitor. Coach. Win.</h2>
